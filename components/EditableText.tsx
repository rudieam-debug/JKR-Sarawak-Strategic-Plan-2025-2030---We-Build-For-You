...
import { Edit, Save, X, Bot } from 'lucide-react';
...
export const EditableText: React.FC<EditableTextProps> = ({ isAdminMode, initialValue, onSave, as, isTextarea, label, textClassName, inputClassName, onSummarize }) => {
  ...
  const colorId = label.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '');
  ...
  return (
    <div
      className={`relative group ${isAdminMode && !isEditing ? 'p-1 -m-1 border border-dashed border-transparent hover:border-blue-400 dark:hover:border-blue-600 rounded-md transition-colors' : ''}`}
    ...
    >
      ...
      <div className="absolute top-0 right-0 flex items-center space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
        {onSummarize && !isEditing && (
            <button onClick={onSummarize} className="p-1 rounded-full bg-blue-100 hover:bg-blue-200 dark:bg-blue-900 dark:hover:bg-blue-800 text-blue-500 dark:text-blue-300" title={`Summarize ${label}`}>
              <Bot className="w-3 h-3" />
            </button>
        )}
        {isAdminMode && (
          <button onClick={() => setIsEditing(true)} className="p-1 rounded-full bg-blue-100 hover:bg-blue-200 dark:bg-blue-900 dark:hover:bg-blue-800 text-blue-500 dark:text-blue-300" title={`Edit ${label}`}>
