...
import { getImages, addImage, deleteImage, updateImageName, blobToBase64 } from '../utils/mediaLibrary';
...
export const MediaLibraryModal: React.FC<MediaLibraryModalProps> = ({ isOpen, onClose, onSelectImage }) => {
  ...
  useEffect(() => {
    const initializeMediaLibrary = async () => {
      ...
      const defaultLogoName = 'JKR Sarawak Logo.png';
      const defaultLogoId = 'default-logo-jkr-sarawak';
      const hasDefaultLogo = currentImages.some(img => img.id === defaultLogoId);

      if (!hasDefaultLogo) {
        try {
          const response = await fetch('https://ik.imagekit.io/z7yhsbzej/7227f37e-0954-4095-88b0-0c8b05995132%20(1).png');
          const blob = await response.blob();
          const data = await blobToBase64(blob);
          const defaultLogoItem: MediaItem = {
            id: defaultLogoId,
          ...
          // Prepend default logo to the start of the array
          const updatedImages = [defaultLogoItem, ...currentImages]; 
          localStorage.setItem('jkr-media-library', JSON.stringify(updatedImages));
          currentImages = updatedImages;
        } catch (e) {
          console.error("Could not preload default logo into media library:", e);
        }
      }
  ...
  const handleDelete = (id: string, name: string) => {
    if (id === 'default-logo-jkr-sarawak') {
        alert("The default logo cannot be deleted from the media library.");
        return;
    }
  ...
  const handleEditName = (id: string, currentName: string) => {
    if (id === 'default-logo-jkr-sarawak') {
        alert("The default logo's name cannot be changed.");
        return;
    }
    setEditingId(id);
  ...
  const filteredImages = images.filter(img => 
    img.name.toLowerCase().includes(searchQuery.toLowerCase())
      ...
      if (a.id === 'default-logo-jkr-sarawak') return -1;
      if (b.id === 'default-logo-jkr-sarawak') return 1;
